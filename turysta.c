...
int wiek = 0;
int opiekun=0;
int semid_most_wchodzenie, semid_most, semid_turysta_wchodzenie, semid_klatka_pierwsza, semid_klatka_druga, semid_wejscie_do_parku, semid_prom_check;
...
int main() {
	...
	key_t key_wejscie_do_parku = ftok(".", 98);
	if ((semid_wejscie_do_parku = semget(key_wejscie_do_parku, 1, IPC_CREAT | 0600)) == -1) {
		perror("Błąd przy tworzeniu semafora wejscia");
		exit(1);
	}
	...
	key_t key_klatka_pierwsza = ftok(".", 206);
	if ((semid_klatka_pierwsza = semget(key_klatka_pierwsza, 1, IPC_CREAT | 0600)) == -1) {
		perror("Błąd przy tworzeniu semafora klatka_pierwsza");
		exit(1);
	}
	key_t key_klatka_druga = ftok(".", 207);
	if ((semid_klatka_druga = semget(key_klatka_druga, 1, IPC_CREAT | 0600)) == -1) {
		perror("Błąd przy tworzeniu semafora klatka_druga");
		exit(1);
	}
	...
	union semun arg;
	arg.val = KLATKA;
		semctl(semid_klatka_pierwsza, 0, SETVAL, arg);
		semctl(semid_klatka_druga, 0, SETVAL, arg);
	arg.val = 3;
...
// Początek symulacji turysty
	...
	// ---- Przewodnik ----
		...
		if (msgrcv(IDkolejki, (struct msgbuf *)&kom, MAX, id_turysta, 0) == -1) {
			...
			sscanf(kom.mtext, "OK %d", &opiekun);
			if(opiekun==1){
				printf(YEL"[Turysta %d] zdecydował się być opiekunem!\n"RESET, id_turysta);
			}
...
// --- Rozpoczęcie mostu wiszącego
	...
	if(wiek<15){
		...
		printf("[Turysta %d]: Wchodzę na most towarzysząc potrzebującym opiekuna turystom...\n", id_turysta);
	}else{
...
// --- Rozpoczęcie wchodzenia na wieżę widokową
	...
	if(wiek<=5){
		...
		printf("[Turysta %d]: Wchodzę na wieżę towarzysząc potrzebującym opiekuna turystom...\n", id_turysta);
	}else{
		semafor_operacja(semid_klatka_pierwsza, -1);
		...
		if(shm_ptr->wieza_sygnal[numer] == 1){
			...
			// Turysta schodzi z wieży drugą klatką schodową
				if(shm_ptr->wieza_sygnal[numer] == 1){
					...
					semafor_operacja(semid_klatka_druga, -1);
...
// --- Rozpoczęcie płynięcia promem
	...
	if(wiek<15){
		...
		printf("[Turysta %d]: Wchodzę na prom towarzysząc potrzebującym opiekuna turystom...\n", id_turysta);
	}else{
